...
Each node also reports hardware capability hints for future distributed
runtimes: CPU, NVIDIA/CUDA candidates, Apple/Metal candidates, and whether the
node can participate as a distributed worker or coordinator. These hints do not
change current Ollama or LM Studio routing and do not represent a shared
operating-system CPU, GPU, or VRAM device.
