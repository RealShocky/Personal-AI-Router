...
The separate distributed compute fabric is an explicit, opt-in runtime plane.
It coordinates supported CPU/CUDA/Metal workers but does not create a
transparent operating-system GPU or CPU.
...
## Seeing the complete fleet

On the Windows coordinator, launch **NVIDIA Personal AI Router** and open
**Overview**. The regular node cards show PAIR engines and models. The
**Distributed inference fabric** card shows the compute plane: worker state,
runtime, backend, free host memory, heartbeat latency, checkpoint support, jobs,
and executions. It refreshes automatically every five seconds. A worker marked
`ready` is paired and healthy; it does not necessarily have a model loaded.

For deployment of headless workers and portable bundles, continue with the
[fabric operations runbook](fabric-operations.mdx). The [documentation map](README.md)
lists every operator and developer guide.
