...
| `14324` | Authenticated distributed compute fabric |
...
The distributed fabric is a separate runtime plane from ordinary proxy routing.
The Windows PAIR node can act as a coordinator while paired Linux, macOS, and
Windows nodes contribute CPU, CUDA, or Metal workers. Workers authenticate with
the existing cluster identity, send leases and heartbeats, and are removed from
new assignments before quarantine when their lease expires. A fresh handshake
and capability probe are required before they rejoin.

The fabric can make a supported model runtime appear as one logical inference
engine, but it does not create a transparent shared Windows CPU, GPU, or VRAM
device. Distributed jobs use explicit runtime adapters, epochs, and checkpoints;
a worker failure resumes only when the adapter can restore safely. Otherwise the
job fails explicitly instead of duplicating output. Mixed CUDA/Metal/CPU groups
are opt-in and require a compatibility and network-performance probe.

PAIR continues to use Electron as the operator UI. WinUI 3 is not required for
the coordinator or worker services, and custom CUDA kernels or DLLs are added
only after profiling identifies a runtime bottleneck.
