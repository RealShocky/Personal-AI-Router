...
Distributed-fabric development currently uses the existing Electron desktop on
Windows as the operator surface. WinUI 3 is not a prerequisite. The coordinator
and worker services are native Go processes; runtime-specific native libraries
are added only behind an adapter after profiling and ABI tests.
