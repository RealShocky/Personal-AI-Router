...
for /f "delims=" %%V in ('jq -r --arg k "nvpair-compute-fabric"   ".components[$k]" "%VERSIONS_FILE%"')            do set "V_FABRIC=%%V"
...
echo  nvpair-compute-fabric= %V_FABRIC%
...
echo [14/14] Building nvpair-compute-fabric (v%V_FABRIC%)...
cd /d "%ROOT%nvpair-compute-fabric"
go build -ldflags "-X main.Version=%V_FABRIC%" -o nvpair-compute-fabric.exe . || goto :fail
echo       OK
...
copy /y "%ROOT%nvpair-compute-fabric\nvpair-compute-fabric.exe" "%BIN_OUT%\nvpair-compute-fabric.exe" >nul || goto :fail
