...
@('PAIR Fabric portable Windows worker','Run .\run-fabric-worker.ps1 after setting the required PAIR_* environment variables.','For WSL2 workers, run .\keepalive-wsl-worker.ps1 -Distro Ubuntu to keep the distro alive across idle periods.','This bundle contains no cluster identity, certificates, private keys, or models.') | Set-Content (Join-Path $stage 'README.txt') -Encoding utf8
